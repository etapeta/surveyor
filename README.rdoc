...
The result of the editing of a survey rendering is a hierarchical Hash.

Three object types can be associated to a survey:
...
This (as well as other rendering options, like obligatoriness and visibility) can
...
== TO DO LIST
X use "render" name for rendering of html coder (or renderer)
X validation rules on element names (also :required and :regexp options)
O check effective use of options
O :size option for string element
O :show mode
O labels internationalization and :i18n (or :label) option handling (also error messages)
O :class option
O tip management and :tip option
O survey error messages and mistaken fields identification
O survey sheets
O calendar element
O text area element
O checkbox element
O rating element
O list-selector element (with :other option)
O radio-selector element (with :other option)
O file uploader element
O :renderer option
O rules for [:validation, :hiding, :editing]
O :reject_if option
O :hide_if option
O :readonly_if option
O survey nesting within an external form
O :limit option for multipliers
O :no_add option for multipliers
O :no_delete (or :no_resize) option for multipliers
O hob as activerecord (or anyway an ActiveRecord container for hobs)
O fill-in-the-blanks element
O survey collating on single objects rather than hobs
O :design mode for surveys
O quiz evaluation
O percent element
O slider element
O spinner element
O multiple surveys in the same page
