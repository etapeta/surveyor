...
_ check effective use of options
_ :size option for string element
_ :show mode
_ labels internationalization and :i18n (or :label) option handling (also error messages)
_ :class option
_ tip management and :tip option
_ survey error messages and mistaken fields identification
_ survey sheets
_ calendar element
_ text area element
_ checkbox element
_ rating element
_ list-selector element (with :other option)
_ radio-selector element (with :other option)
_ file uploader element
_ :renderer option
_ rules for [:validation, :hiding, :editing]
_ :reject_if option
_ :hide_if option
_ :readonly_if option
_ survey nesting within an external form
_ :limit option for multipliers
_ :no_add option for multipliers
_ :no_delete (or :no_resize) option for multipliers
_ hob as activerecord (or anyway an ActiveRecord container for hobs)
_ fill-in-the-blanks element
_ survey collating on single objects rather than hobs
_ :design mode for surveys
_ quiz evaluation
_ percent element
_ slider element
_ spinner element
_ multiple surveys in the same page
